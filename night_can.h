...
typedef struct NightCANPacket {
    ...
    uint32_t tx_interval_ms; // Transmission interval in milliseconds (0 for one-shot)
...
/**
 * @brief Structure to hold all state information for a single CAN driver instance.
 */
typedef struct {
 ...
 * @param instance Pointer to the NightCANDriverInstance structure for this CAN peripheral.
 * @param hcan Pointer to the HAL CAN handle (e.g., &hfdcan1 for H7, &hcan1 for L4).
 * This handle must be initialized by the STM32CubeMX generated code
 ...
 * Performs direct memory access via pointer casting.
 * @param T The integer type (e.g., int8_t, uint16_t, int32_t) to read as.
 * @param packet_ptr Pointer to the CAN packet structure (e.g., NightCANReceivePacket* or NightCANPacket*).
...
#define can_readInt(T, packet_ptr, start_byte) \
    ( *( (T *)( (uint8_t *)((packet_ptr)->data) + (start_byte) ) ) ) // Cast data to uint8_t* for pointer arithmetic

/**
 * @brief Write an integral value (e.g., int16_t, uint32_t) to a CAN packet's data buffer.
 * Performs direct memory access via pointer casting. Typically used with NightCANPacket*.
 * @param T The integer type (e.g., int8_t, uint16_t, int32_t) to write as.
 * @param packet_ptr Pointer to the CAN packet structure (e.g., NightCANPacket*).
 * @param start_byte The starting byte index (0-7) within the 'data' buffer.
 * @param value The integral value to write into the buffer.
 * @warning See general warnings in file header. Does NOT update packet_ptr->dlc.
 * Example: can_writeInt(int8_t, &myTxPacket, 0, -10);
 * myTxPacket.dlc = 1; // Manually set DLC
 */
#define can_writeInt(T, packet_ptr, start_byte, value) \
    ( *( (T *)( (uint8_t *)((packet_ptr)->data) + (start_byte) ) ) = (value) ) // Cast data to uint8_t*

/**
 * @brief Write a floating point value to a CAN packet's data buffer after scaling and conversion.
 * Scales the float by the inverse of precision, converts to integer type T,
 * and writes it using direct memory access. Typically used with NightCANPacket*.
 * @param T The target integer type (e.g., int16_t, uint16_t) to store the value as in the buffer.
 * @param packet_ptr Pointer to the CAN packet structure (e.g., NightCANPacket*).
 * @param start_byte The starting byte index (0-7) within the 'data' buffer.
 * @param value The floating-point value to write.
 * @param precision The scaling factor (resolution). 'value' is divided by 'precision' before conversion.
 * Example: Store 4.85V with 0.01V precision as uint16_t at byte 2:
 * can_writeFloat(uint16_t, &myTxPacket, 2, 4.85f, 0.01f);
 * // Stored integer value will be (uint16_t)(4.85 / 0.01) = 485
 * myTxPacket.dlc = 4; // Manually set DLC (assuming bytes 0,1 also used)
 * @warning See general warnings in file header. Does NOT update packet_ptr->dlc. Ensure 'precision' is not zero.
 */
#define can_writeFloat(T, packet_ptr, start_byte, value, precision) \
    ( *( (T *)( (uint8_t *)((packet_ptr)->data) + (start_byte) ) ) = (T)( (value) / (precision) ) ) // Cast data to uint8_t*

/**
 * @brief Read a floating-point value from a CAN packet's data buffer, assuming it was stored scaled.
 * Reads an integer of type T from the buffer and scales it by 'precision'
 * to reconstruct the original floating-point value.
 * @param T The underlying integer type (e.g., int16_t, uint16_t) that was stored in the buffer.
 * @param packet_ptr Pointer to the CAN packet structure (e.g., NightCANReceivePacket* or NightCANPacket*).
 * @param start_byte The starting byte index (0-7) within the 'data' buffer where the integer is stored.
 * @param precision The scaling factor (resolution) that was used when the float was originally stored.
 * Example: Read value stored with 0.01 precision as uint16_t at byte 2:
 * float voltage = can_readFloat(uint16_t, &myReceivedPacket, 2, 0.01f);
...
#define can_readFloat(T, packet_ptr, start_byte, precision) \
    ( (float)( *( (T *)( (uint8_t *)((packet_ptr)->data) + (start_byte) ) ) * (precision) ) ) // Cast data to uint8_t*
